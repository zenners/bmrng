...
YAML.load(ENV['ROLES']).each do |role|
  Role.find_or_create_by_name!( :without_protection => true)
  puts 'role: ' << role
end
puts 'DEFAULT USERS'
...
puts 'user: ' << user.name
user.add_role :admin
user2 = User.create! :name => 'Silver User', :email => 'user2@example.com', :password => 'changeme', :password_confirmation => 'changeme'
